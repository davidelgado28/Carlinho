...
export default function Browser() {
  ...
  const handleNav = (e: React.FormEvent) => {
    e.preventDefault();
    if (!input.startsWith('http')) setUrl('https://' + input);
    else setUrl(input);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <form onSubmit={handleNav} className="flex p-2 bg-gray-200 gap-2 border-b">
        ...
        />
        <button className="bg-blue-500 text-white px-4 py-1 rounded">Ir</button>
      </form>
      <iframe src={url} className="flex-1 w-full border-none" sandbox="allow-scripts allow-same-origin" />
    </div>
  );
}
